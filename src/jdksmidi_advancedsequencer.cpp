 ...
 * this #include chain: midi.h -> world.h -> utils.h -> multitrack.h -> track.h
 * -> msg.h -> midi.h
 * I suggest to separate MIDIMultiTrack declarations in utils.h from general
 * utility functions
 */
...

{
     ...
     * close it in the dtor:
     * is this right? perhaps we should open/close only in Play/Stop (but what
     * if thru is enabled?)
     * What is better?
     */
...
bool AdvancedSequencer::Load( const char *fname )
{
    ...
    if ( reader.Parse() )
    {
        file_loaded = true;
        Reset();
        // GoToMeasure ( 0 ); OLD: it used warp_positions, not even
        // initialized!!! However,
...
void AdvancedSequencer::Play()
{
    ...
    CatchEventsBefore();
    // this intercepts any CC, SYSEX and TEMPO messages and send them to the out
    // port
    // allowing to start with correct values; we could incorporate this in the
    // sequencer state, but it would track even CC (not difficult) and SYSEX
    // messages
...
void AdvancedSequencer::OutputMessage( MIDITimedBigMessage &msg )
{
    for ( int i = 1; i < 1000; i++ ) // retry for 1 second
    {
        ...
        jdks_wait( 1 );
        // note by NC: this may not be accurate, however we only want to wait
...
/* NOTE BY NC: soloing and muting has been enhanced to keep count of muted CC,
 * SYSEX changes previously muted
 * so when we unmute a track it sounds with correct parameters even if they
 * weren't tramsmitted before
...
{ // new by NC
    ...
    if ( mgr->IsSeqPlay() )
    {
        ...
        double ms_per_clock = 60000.0 / ( seq->GetState()->tempobpm * seq->GetCurrentTempoScale() * tracks->GetClksPerBeat() );
...
void AdvancedSequencer::ExtractMarkers ( std::vector< std::string > *list )
{
    ...
    for ( int i = 0; i < t->GetNumEvents(); ++i )
    {
        ...
        if ( m )
        {
            ...
            long beats_gone_by = ( m->GetTime() - last_beat_time ) /
clks_per_beat;
            ...
            if ( m->IsMetaEvent() && m->IsTimeSig() )
            {
                ...
                clks_per_beat = tracks.GetClksPerBeat() * 4 /
timesig_denominator;
            }
            ...
            if ( m->IsTextEvent() && m->GetSysEx() )
            {
                        ...
                        || m->GetMetaType() == META_CUE_POINT )
                {
                    char buf[256];
                    char line[256];
                    memcpy ( buf, m->GetSysEx()->GetBuf(),
m->GetSysEx()->GetLengthSE() );
                    buf[ m->GetSysEx()->GetLengthSE() ] = '\0';
                    FixQuotes ( buf );
                    sprintf ( line, "%03d:%d        %s", measure + 1, beat + 1,
buf );
...
void AdvancedSequencer::ExtractWarpPositions()
{
    ...
    Stop();
    // warp_positions is now a vector of objects ( not pointers ) so we can
    // minimize memory dealloc/alloc
...
void AdvancedSequencer::CatchEventsBefore()
{
    ...
    while ( iter.GetCurEvent( &trk, &msgp ) && msgp->GetTime() < seq->GetCurrentMIDIClockTime() )
    {
        ...
        else if ( msg.IsSystemExclusive() ) // TODO: which SysEx should we send?
        {
            OutputMessage( msg );
            jdks_wait( 10 );
        }
